...
export default function SuperAdmin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:3001/api/admin/tenants/${editingId}`, formData);
      } else {
        await axios.post('http://localhost:3001/api/admin/tenants', formData);
      }
      setShowModal(false);
      fetchTenants();
    } catch (err) {
  ...
  const toggleStatus = async (tenant) => {
    try {
      ...
      await axios.put(`http://localhost:3001/api/admin/tenants/${tenant.id}`, { status: newStatus });
      fetchTenants();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/admin/tenants/${id}`);
      fetchTenants();
    } catch (err) {
