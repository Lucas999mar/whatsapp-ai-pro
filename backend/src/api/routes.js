...
// ── ADMIN MIGRATION (temporary – remove after first run) ──────
router.get('/admin/migrate-tenant-users', async (req, res) => {
  try {
    const supabase = getSupabase();
    // Check if table already exists
    const { data: check, error: checkErr } = await supabase.from('tenant_users').select('id').limit(1);
    if (!checkErr) {
      return res.json({ status: 'ok', message: 'Tabela tenant_users já existe.' });
    }
    if (checkErr && checkErr.code !== '42P01' && !checkErr.message.includes('does not exist')) {
      return res.json({ status: 'exists_or_other', message: checkErr.message });
    }

    // Table doesn't exist – create it via direct Postgres connection if DATABASE_URL is available
    ...
    if (dbUrl) {
      ...
      `);
      await client.end();
      return res.json({ status: 'ok', message: '✅ Tabela tenant_users criada com sucesso via DATABASE_URL!' });
    }

    // Fallback: try Supabase SQL API
    ...
    // Try various SQL endpoints
    const endpoints = [
      { url: `${supabaseUrl}/pg/query`, body: { query: sqlQuery } },
      { url: `${supabaseUrl}/rest/v1/rpc/run_sql`, body: { sql: sqlQuery } },
    ];
    for (const ep of endpoints) {
      try {
        const r = await axios.post(ep.url, ep.body, { headers, timeout: 15000 });
        return res.json({ status: 'ok', message: `Migração via ${ep.url} executada!`, data: r.data });
      } catch (e) { /* try next */ }
    }

    return res.status(500).json({ status: 'error', message: 'Nenhum método de SQL direto disponível. Configure DATABASE_URL no Render ou execute manualmente no Supabase SQL Editor.' });
  } catch (err) {
